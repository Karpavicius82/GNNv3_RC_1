...
constexpr int kInputs = 4;      // center self-state + 3 neighbor messages
constexpr int kModes = 4;       // updated center port + 3 residual modes
...
Mat completeUnitaryFromUpdateRow(const Vec& update_row) {
    Mat rows;
    rows.push_back(normalized(update_row));
...
struct NodeParams {
    double self_gain = 1.20;
    double neighbor_gain[kNeighbors] = {0.90, 0.70, 0.50};
    double phase[kNeighbors] = {0.11 * kPi, -0.27 * kPi, 0.43 * kPi};
};

Vec updateRow(const NodeParams& p) {
    Vec row(kInputs, cd(0, 0));
    row[0] = cd(p.self_gain, 0);
    for (int i = 0; i < kNeighbors; ++i) {
        row[1 + i] = p.neighbor_gain[i] * std::exp(cd(0, p.phase[i]));
    }
    return normalized(row);
}

Mat nodeUpdateKernel(const NodeParams& p) {
    return completeUnitaryFromUpdateRow(updateRow(p));
}

Medium nodeUpdateMedium(const Mat& w) {
    Medium m(kNodes);
...
Medium nodeUpdateMedium(const NodeParams& p) {
    return nodeUpdateMedium(nodeUpdateKernel(p));
}
...
bool testUnitaryNodeUpdateFrame() {
    std::printf("\n[1] UNITARY NODE-UPDATE TRANSFER FRAME\n");
    const NodeParams p = defaultParams();
    const Mat w = nodeUpdateKernel(p);
    const Medium m = nodeUpdateMedium(w);
...
bool testSelfAndNeighborWeightsAreVisible() {
    std::printf("\n[2] SELF AND EDGE COUPLINGS SET UPDATE CONTRIBUTIONS\n");
    const NodeParams p = defaultParams();
    const Mat w = nodeUpdateKernel(p);
    const Medium m = nodeUpdateMedium(w);
    bool pass = true;

    std::vector<double> expected_power(kInputs, 0.0);
    for (int i = 0; i < kInputs; ++i) expected_power[i] = std::norm(w[0][i]);

    std::vector<double> observed_power(kInputs, 0.0);
    for (int i = 0; i < kInputs; ++i) {
        Vec input(kInputs, cd(0, 0));
        input[i] = cd(1, 0);
        const Vec out = routeInput(m, input);
        observed_power[i] = std::norm(out[0]);
        pass = pass && std::abs(observed_power[i] - expected_power[i]) < 1e-12;
    }

    std::printf("    update power self=%.12f n0=%.12f n1=%.12f n2=%.12f\n",
                observed_power[0], observed_power[1], observed_power[2], observed_power[3]);
    std::printf("    expected      self=%.12f n0=%.12f n1=%.12f n2=%.12f\n",
                expected_power[0], expected_power[1], expected_power[2], expected_power[3]);
    std::printf("    => %s\n", pass ? "PASS" : "FAIL");
    return pass;
}

bool testEdgePhasesControlNeighborMessage() {
    std::printf("\n[3] EDGE PHASES CONTROL NEIGHBOR MESSAGE INTERFERENCE\n");
    NodeParams aligned;
    aligned.self_gain = 0.0;
    aligned.neighbor_gain[0] = 1.0;
    aligned.neighbor_gain[1] = 1.0;
    aligned.neighbor_gain[2] = 1.0;
    aligned.phase[0] = 0.0;
    aligned.phase[1] = 0.0;
    aligned.phase[2] = 0.0;

    NodeParams destructive = aligned;
    destructive.phase[0] = 0.0;
    destructive.phase[1] = 2.0 * kPi / 3.0;
    destructive.phase[2] = 4.0 * kPi / 3.0;

    const Vec neighbor_equal = normalized({cd(0, 0), cd(1, 0), cd(1, 0), cd(1, 0)});
    const Vec aligned_out = routeInput(nodeUpdateMedium(aligned), neighbor_equal);
    const Vec destructive_out = routeInput(nodeUpdateMedium(destructive), neighbor_equal);
    const double aligned_update = std::norm(aligned_out[0]);
    const double aligned_residual = portPower({aligned_out[1], aligned_out[2], aligned_out[3]});
    const double destructive_update = std::norm(destructive_out[0]);
    const double destructive_residual = portPower({destructive_out[1], destructive_out[2], destructive_out[3]});
    const bool pass = aligned_update > 0.999999 && aligned_residual < 1e-12 &&
                      destructive_update < 1e-12 && destructive_residual > 0.999999;

    std::printf("    aligned update=%.12f residual=%.3e\n", aligned_update, aligned_residual);
    std::printf("    destructive update=%.3e residual=%.12f\n", destructive_update, destructive_residual);
    std::printf("    => %s\n", pass ? "PASS" : "FAIL");
    return pass;
}

bool testPermutationEquivarianceForNeighborLabels() {
    std::printf("\n[4] NODE UPDATE IS EQUIVARIANT TO NEIGHBOR RELABELING\n");
    const NodeParams p = defaultParams();
    const Vec input = normalized({cd(0.6, -0.1), cd(0.4, -0.2), cd(-0.1, 0.8), cd(0.5, 0.3)});
    const int perm[kNeighbors] = {2, 0, 1};
    NodeParams pp = p;
    Vec input_p(kInputs, cd(0, 0));
    input_p[0] = input[0];
    for (int i = 0; i < kNeighbors; ++i) {
        pp.neighbor_gain[i] = p.neighbor_gain[perm[i]];
        pp.phase[i] = p.phase[perm[i]];
        input_p[1 + i] = input[1 + perm[i]];
    }

    const Vec out = routeInput(nodeUpdateMedium(p), input);
    const Vec out_p = routeInput(nodeUpdateMedium(pp), input_p);
    const double update_amp_err = std::abs(out[0] - out_p[0]);
    const double update_power_err = std::abs(std::norm(out[0]) - std::norm(out_p[0]));
    const bool pass = update_amp_err < 1e-12 && update_power_err < 1e-12;

    std::printf("    updated-center amplitude error=%.2e power error=%.2e\n",
                update_amp_err, update_power_err);
    std::printf("    residual basis may rotate under relabeling\n");
    std::printf("    => %s\n", pass ? "PASS" : "FAIL");
    return pass;
}

bool testResidualModesPreventFalseCompression() {
    std::printf("\n[5] RESIDUAL MODES PREVENT FALSE NODE-STATE COMPRESSION\n");
    const NodeParams p = defaultParams();
    const Mat w = nodeUpdateKernel(p);
    const Medium m = nodeUpdateMedium(w);
    const Vec neighbor_basis = {cd(0, 0), cd(0, 0), cd(1, 0), cd(0, 0)};
    const Vec out = routeInput(m, neighbor_basis);
    const double update = std::norm(out[0]);
    const double residual = std::norm(out[1]) + std::norm(out[2]) + std::norm(out[3]);
    const double total = portPower(out);
    const double expected_update = std::norm(w[0][2]);
    const bool pass = std::abs(update - expected_update) < 1e-12 &&
                      std::abs(residual - (1.0 - expected_update)) < 1e-12 &&
                      std::abs(total - 1.0) < 1e-12;

    std::printf("    basis neighbor update=%.12f residual_modes=%.12f total_output=%.12f\n",
                update, residual, total);
    std::printf("    no_compression_boundary=CONFIRMED\n");
    std::printf("    => %s\n", pass ? "PASS" : "FAIL");
    return pass;
}

bool testGaugeCovariantNodeUpdate() {
    std::printf("\n[6] NODE UPDATE TRANSFER IS GAUGE-COVARIANT\n");
    const Medium m = nodeUpdateMedium(defaultParams());
...
bool testRandomDisorderKeepsUpdateBounded() {
    std::printf("\n[7] RANDOM DISORDER KEEPS NODE UPDATE SIGNAL BOUNDED\n");
    const Medium ideal = nodeUpdateMedium(defaultParams());
    const Vec input = normalized({cd(0.6, -0.1), cd(0.4, -0.2), cd(-0.1, 0.8), cd(0.5, 0.3)});
    ...
    struct Row {
        ...
        double min_update_power = std::numeric_limits<double>::infinity();
    ...
    for (auto& row : rows) {
        for (int seed = 0; seed < 24; ++seed) {
            const Medium perturbed = perturbRandomDisorder(ideal, row.eps, row.eps, 0xC0FFEEu + seed * 97u);
            const Vec full = evolvePhysical(perturbed, input);
            const Vec out = extractOutput(full);
            const double update_err = std::abs(out[0] - ideal_out[0]);
            row.mean_update_err += update_err;
            row.max_update_err = std::max(row.max_update_err, update_err);
            row.min_update_power = std::min(row.min_update_power, std::norm(out[0]));
    ...
    for (const auto& row : rows) {
        ...
        std::printf("    random eps=%.3f seeds=24 mean_update_err=%.4f max_update_err=%.4f min_update_power=%.4f max_input_residual=%.4f %s\n",
                    row.eps, row.mean_update_err, row.max_update_err, row.min_update_power,
...
int main() {
    std::printf("=====================================================================\n");
    std::printf(" GRAPH-WAVE NODE UPDATE CONTRACT TEST\n");
    std::printf(" GNN-analog: self-state + neighbor messages -> updated center + residual modes.\n");
    std::printf(" No V26 runtime coupling, no legacy GA/readout, no trainer, no production decision.\n");
    std::printf("=====================================================================\n");

    int pass = 0;
    const int total = 7;
    pass += testUnitaryNodeUpdateFrame() ? 1 : 0;
    pass += testSelfAndNeighborWeightsAreVisible() ? 1 : 0;
    pass += testEdgePhasesControlNeighborMessage() ? 1 : 0;
    pass += testPermutationEquivarianceForNeighborLabels() ? 1 : 0;
    pass += testResidualModesPreventFalseCompression() ? 1 : 0;
    pass += testGaugeCovariantNodeUpdate() ? 1 : 0;
    pass += testRandomDisorderKeepsUpdateBounded() ? 1 : 0;

    std::printf("\n=====================================================================\n");
    std::printf(" RESULT : %d / %d verified\n", pass, total);
    std::printf(" %s\n", pass == total
                    ? "node update is a GNN-analog substrate primitive with explicit residual modes"
                    : "node update contract is not stable enough yet");
    std::printf("=====================================================================\n");
